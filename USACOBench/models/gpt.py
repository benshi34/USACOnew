...
async def generate_from_openai_chat_completion(
    ...
    **kwargs,
    ...
    if 'gpt' in model.lower():
        ...
        client = AsyncOpenAI(api_key=os.environ.get("TOGETHER_API_KEY"), base_url="https://api.together.xyz/v1")
    async_responses = []
...
def gpt(prompt, model="gpt-4", temperature=0.2, max_tokens=2000, n=1, stop=None, **kwargs) -> list:
    return gpts([prompt] * n, model=model, temperature=temperature, max_tokens=max_tokens, stop=stop, **kwargs)[0]

def gpts(prompts, model="gpt-4", temperature=0.2, max_tokens=2000, stop=None,
...
def chatgpt(messages, model="gpt-4", temperature=0.2, max_tokens=2000, n=1, stop=None, **kwargs) -> list:
    return chatgpts([messages] * n, model=model, temperature=temperature, max_tokens=max_tokens, stop=stop, **kwargs)[0]

def chatgpt_raw(messages, model="gpt-4", temperature=0.2, max_tokens=2000, n=1, stop=None, **kwargs) -> list:
    return chatgpts_raw([messages] * n, model=model, temperature=temperature, max_tokens=max_tokens, stop=stop, **kwargs)[0]

def chatgpts(messages_list, model="gpt-4", temperature=0.2, max_tokens=2000, stop=None, max_messages=200, **kwargs) -> list:
    texts = []
...
def chatgpts_raw(messages_list, model="gpt-4", temperature=0.2, max_tokens=2000, stop=None, max_messages=200, **kwargs) -> list:
    '''
...
def claude(prompts, model="claude-3-5-sonnet-20240620", temperature=0.2, max_tokens=3000, stop=None, max_messages=200, system_prompt=None, **kwargs) -> list:
    texts = []
    if system_prompt is not None:
        messages_list = [[{'role': 'system', 'content': system_prompt},
                          {"role": "user", "content": prompt}] for prompt in prompts]
